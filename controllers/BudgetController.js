...
const FILENAME = 'BudgetController.js';
...
exports.ReportBudget = async function (req,res) {
    var timeoutVoidTransactionId = Math.floor(Math.random() * (1000000000 - 1000 + 1) + 1000);
    const functionname = 'updateCompany';
    try {
        const data = req.body;
        ...
        const { idBudget} = data;
        const pool = await sqlDriver.getPool();   
        let query = `SELECT * FROM CtaPresupuesto cp 
        INNER JOIN Presupuesto p ON cp.pkfk_Presupuesto = p.pk_Presupuesto 
        WHERE p.pk_Presupuesto = ${idBudget}`;        
            const result = await pool.request().query(query);
            return responseManager.sendResponseWithDocument(res,result);
    } catch (error) {
        return errorManager.handleTransactionCatchClause(error, FILENAME, functionname, 500, res, 'Error en login');
...
getPresupuesto = async function(idBudget){
    try {
        const pool = await sqlDriver.getPool();   
        let query = `SELECT * FROM CtaPresupuesto cp 
        INNER JOIN Presupuesto p ON cp.pkfk_Presupuesto = p.pk_Presupuesto 
